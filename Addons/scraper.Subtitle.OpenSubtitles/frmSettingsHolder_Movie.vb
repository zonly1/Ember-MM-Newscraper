...
#Region "Methods"
    ...
    Private Sub btnDown_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDown.Click
        Dim order As Integer = ModulesManager.Instance.externalScrapersModules_Subtitle_Movie.FirstOrDefault(Function(p) p.AssemblyName = OpenSubtitle_Subtitle._AssemblyName).ModuleOrder
        If order < ModulesManager.Instance.externalScrapersModules_Subtitle_Movie.Count - 1 Then
            ModulesManager.Instance.externalScrapersModules_Subtitle_Movie.FirstOrDefault(Function(p) p.ModuleOrder = order + 1).ModuleOrder = order
            ModulesManager.Instance.externalScrapersModules_Subtitle_Movie.FirstOrDefault(Function(p) p.AssemblyName = OpenSubtitle_Subtitle._AssemblyName).ModuleOrder = order + 1
    ...
    Sub orderChanged()
        Dim order As Integer = ModulesManager.Instance.externalScrapersModules_Subtitle_Movie.FirstOrDefault(Function(p) p.AssemblyName = OpenSubtitle_Subtitle._AssemblyName).ModuleOrder
        If ModulesManager.Instance.externalScrapersModules_Subtitle_Movie.Count > 0 Then
            btnDown.Enabled = (order < ModulesManager.Instance.externalScrapersModules_Subtitle_Movie.Count - 1)
